...
rem   %USERPROFILE%\Zomboid\Workshop\PhunServer2     - upload staging
rem   %USERPROFILE%\Zomboid\Workshop\PhunServer2Test - test upload staging
rem ---------------------------------------------------------------------------

set MODS=PhunServer2 PhunServer2Commands PhunServer2Cron PhunServer2Chat PhunServer2DayNight PhunServer2Wiper

set SRC=%~dp0
set MODDIR=%USERPROFILE%\Zomboid\mods
set WS=%USERPROFILE%\Zomboid\Workshop\PhunServer2
set WSTEST=%USERPROFILE%\Zomboid\Workshop\PhunServer2Test

echo [PhunServer2] Deploying to %MODDIR%

rem --- Live mods -------------------------------------------------------------
for %%M in (%MODS%) do (
    rmdir /S /Q "%MODDIR%\%%M" 2>nul
    xcopy "%SRC%Contents\mods\%%M" "%MODDIR%\%%M" /Y /I /E /F /Q >nul
    if errorlevel 1 echo [PhunServer2] FAILED copying %%M
)
